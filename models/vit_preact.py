...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, feats:int, head:int=8, dropout:float=0.):
        ...
        self.q = nn.Linear(feats, feats, bias=BIAS)
        self.k = nn.Linear(feats, feats, bias=BIAS)
        self.v = nn.Linear(feats, feats, bias=BIAS)

        self.q_la = nn.LayerNorm(self.feats)
        self.k_la = nn.LayerNorm(self.feats)
        self.v_la = nn.LayerNorm(self.feats)

        self.o = nn.Linear(feats, feats, bias=BIAS)
        self.o_la = nn.LayerNorm(self.feats)
    ...
    def forward(self, x):
        ...
        o = self.dropout(self.o_la(self.o(attn.flatten(2))))
        return o
...
class TransformerEncoder(nn.Module):
    def __init__(self, feats:int, mlp_hidden:int, head:int=8, dropout:float=0.):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(feats, mlp_hidden, bias=BIAS),
            nn.LayerNorm(mlp_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(mlp_hidden, feats, bias=BIAS),
            nn.LayerNorm(feats),
            nn.GELU(),
            nn.Dropout(dropout),
        )
...
class ViT_preact(nn.Module):
    def __init__(self, in_c:int=3, num_classes:int=10, img_size:int=32, patch:int=8, dropout:float=0., num_layers:int=7, hidden:int=384, mlp_hidden:int=384*4, head:int=8, is_cls_token:bool=True):
        super(ViT_preact, self).__init__()
        # hidden=384
        ...
        self.emb = nn.Linear(f, hidden, bias=BIAS) # (b, n, f)
        self.emb_la = nn.LayerNorm(hidden)
        ...
        self.fc = nn.Sequential(
            nn.LayerNorm(hidden),
            nn.Linear(hidden, num_classes, bias=BIAS) # for cls_token
        )
