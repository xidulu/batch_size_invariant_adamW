...
if args.use_std_adam:
    optimizer = optim.AdamW(net.parameters(), lr=LR, betas=(beta1, beta2), weight_decay=1e-2)
else: 
    optimizer = AdamBI(net.parameters(), lr=LR, gammas=(gamma1, gamma2), weight_decay=1e-2)
...
if args.wd:
    prefix_string += f"wd_{args.wd}"

if not args.layernorm:
    # Use batchnorm
    prefix_string += 'batchnorm__'
